...
> **Правило `no-module-install`:** регистрацию и снятие агентов через `DoInstall` / `DoUninstall` в `install/` не предлагаем. На проекте — `CAgent::AddAgent` вручную, CLI-команда, sprint.migration или механизм проекта.
...
- Снятие агентов модуля: `CAgent::RemoveModuleAgents('vendor.module')` (на проекте — вручную или скриптом, **не через `install/`**).
...
- [ ] Агенты модуля можно снять через `CAgent::RemoveModuleAgents`; не регистрировать/удалять через `install/` (см. `no-module-install.mdc`).
